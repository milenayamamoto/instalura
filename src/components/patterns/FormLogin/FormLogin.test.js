...
describe('<FormLogin />', () => {
  describe('when form fields are valid', () => {
    test('complete the submission', async () => {
      ...
      user.type(inputUsuario, 'username');
      await waitFor(() => expect(inputUsuario).toHaveValue('username'));

      const inputSenha = screen.getByPlaceholderText('Senha');
      user.type(inputSenha, 'password');
      await waitFor(() => expect(inputSenha).toHaveValue('password'));
